...
def get_explain_grad_args() -> argparse.Namespace:
    """
    Get arguments for the gradient-based explanation process.
    Loads from YAML by default and allows CLI overrides.
    """
    parser = argparse.ArgumentParser(description='Explain a prediction using gradient-based methods')
    
    # Config file path
    parser.add_argument('--config', type=str, default='explain_grad/config/explain_grad_config.yaml',
                        help='Path to the YAML configuration file')
    
    # CLI Overrides
    parser.add_argument('--model_path', type=str, help='Directory to trained model')
    parser.add_argument('--dataset', type=str, help='Dataset name')
    parser.add_argument('--method', type=str, choices=['raw_grad', 'smoothgrad', 'grad_times_input'],
